...
export class EditFavoriteBookScreen extends React.Component {
  static navigationOptions = ({ navigation }) => {
    ...
    constructor(props) {
      super(props);
      this.state = {
        response: this.props.favoriteBook || '',
      }
    };

    handleSave(){
      this.props.dispatch(updateUserInfo('info', 'favoriteBook', this.state.response))
    ...
    return (
      <View style={styles.containerStyle}>
        ...
        <View style={styles.questionContainerStyle}>
          <Text style={styles.questionTextStyle}>Add your Favorite Book:</Text>
          <UnderlinedInput
            onChangeText={(text) => this.setState({response: text})}
            value={this.state.response}
            placeholder='Tanakh ... Obviously.'
            returnKeyType='done'
            autoFocus = {true}
...
export default connect(mapStateToProps)(EditFavoriteBookScreen);
